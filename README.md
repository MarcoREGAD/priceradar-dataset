...
[![Updated twice daily](https://img.shields.io/badge/updated-twice%20daily-4a6fa5)](DEPLOY-COOLIFY.md)
...
| Market | Storefront | Currency | Collection started |
| --- | --- | --- | --- |
| `FR` | Back Market France | EUR | 13 August 2026 |
| `US` | Back Market US | USD | 18 August 2026 |
| `DE` | Back Market Deutschland | EUR | 30 August 2026 |
| `ES` | Back Market España | EUR | 3 September 2026 |

Counts are deliberately not repeated here — they change twice a day and a
hand-written number would be wrong within hours. Current volumes, per market,
are in [`stats.json`](stats.json), regenerated with every update.
...
Everything here is regenerated by four dependency-free Python scripts, driven by
a single shell script. No lockfile, no install step: the standard library is
enough.
...
That regenerates every file and publishes nothing. `PUSH=true` makes it commit
and push, which is what the container does on a schedule.

The updates are produced by a container running on our own server, described in
[DEPLOY-COOLIFY.md](DEPLOY-COOLIFY.md). Local usage is covered in
[SETUP.md](SETUP.md).
