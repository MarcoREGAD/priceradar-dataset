...
Everything here is regenerated by three dependency-free Python scripts, driven
by a single shell script that the scheduled job and the Docker image both call.

```bash
docker compose run --rm update   # regenerate; publishes nothing
```
...
Setup, credentials and the server variant are covered in [SETUP.md](SETUP.md).
